...
class CreateGui:
    def __init__(self,config_subscan_api_info,config_ui_info,config_cryptact_info):
        ...
        reward_slash_data_header_config = f'reward_slash_data_header_{self.__token_data.lower()}'
        self.__reward_slash_data_header_token = config_subscan_api_info[reward_slash_data_header_config].split(',')
    ...
    def main_window(self):
        ...
        show_layout = ( 
            ...
            [
                ...
                sg.Spin(values = self.__token_data_list, font=(font_info,20), initial_value = self.__token_data,size=(4, 1), key = '-TOKEN-'),
                sg.Text('履歴タイプ:',font=(font_info,20),size=(10, 1)),
                sg.Spin(values = self.__history_type_list, font=(font_info,20), initial_value = self.__history_type,size=(12, 1), key = '-HISTORY-'),
                sg.Button('テーブル初期化',font=(font_info,20),size=(10, 1), key = '-RELOAD-'),
            ...
            [
                ...
                sg.Spin(values = self.__sort_list, font=(font_info,20), initial_value = self.__sort_type,size=(5, 1), key = '-SORT-'),
                sg.Button('履歴取得',font=(font_info,20), key = '-SHOW-'),
                sg.Button('クリア',font=(font_info,20),size=(5, 1), key = '-UNSHOW-'),
                sg.Button('csvファイル保存',font=(font_info,20),  disabled = True, key = '-SAVE-')
            ],
            [sg.HorizontalSeparator()]   
        )
        ...
        response_data_layout = (
            ...
            sg.InputText('', font=(font_info,20),use_readonly_for_disable=True, disabled=False, border_width=0, background_color = 'white', key='-TABLE_DATA-')
        )
        ...
        # ウインドウ位置
        return sg.Window('DL Subscan Staking Rewards History',layout,resizable=True,grab_anywhere=True)
    ...
    def config_window(self,config,token):
        ...
        api_key_layout = [
            ...
            sg.Input(config['api_key'], password_char='*', font=(font_info,20),key='-SUBSCAN_API-', size=(30, 1))
        ]
        ...
        address_layout = [
            ...
            sg.Input(config[address_cnfig], font=(font_info,20),key=address_layout_key, size=(47, 1))
        ]
        ...
        adjust_val_layout = [
            ...
            sg.Input(config[adjust_val_cnfig], font=(font_info,20),key=adjust_val_layout_key, size=(30, 1))
        ]
        ...
        display_digit_layout = [
            sg.Text(display_digit_layout_text_value, font=(font_info,20),size=(15, 1)),
            sg.Input(config[display_digit_config], key=display_digit_layout_key, font=(font_info,20),size=(5, 1))
        ]

        config_layout = [
            [sg.Text('Subscan API 設定', font=('游ゴシック Medium',22), justification='center', text_color='#483d8b',pad=(0, 10))],
            [api_key_layout],
            [address_layout],
            [adjust_val_layout],
            [display_digit_layout],
            ...
            [
                sg.Button('更新', font=(font_info,20),key='-CONFIG_UPDATE-', size=(5, 1), pad=((300, 10),(20,20))),
                sg.Button('キャンセル', font=(font_info,20),key='-CANCEL-', size=(8, 1), pad=((10, 300),(20,20)))
            ]
        ]

        return sg.Window('DL Subscan Staking Rewards History', config_layout, finalize=True)
    ...
    def usage_window(self):
        ...
        usage_window_layout = [
            [sg.Text('DL Subscan Staking Rewards History', font=(font_info,25), justification='center', text_color='#483d8b')],
            [sg.Image('./usage.png')],
            [sg.Button('戻る', font=(font_info,20),button_color = ('#000000','#1f3134'),key='-RETURN_MAIN_WINDOW-', size=(5, 1), pad=(180, 20))]
        ]
