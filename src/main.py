...
def main():
    ...
    config_subscan_api_info = config['subscan_api_info']
    config_subscan_api_doc  = config_subscan_api_info['subscan_api_doc']
    config_ui_info          = config['ui_info']
    config_cryptact_info    = config['cryptact_info']

    # CreateGuiクラスのインスタンス作成
    sg_gui = CreateGui(config_subscan_api_info,config_ui_info,config_cryptact_info)
    ...
    while True:
        ...
        if event == '-SHOW-':
            ...
            address_token_value = config_subscan_api_info[address_token]
            decimal_point_adjust_token = f'adjust_value_{(sg_gui.token_data).lower()}'
            decimal_point_adjust_token_value = config_subscan_api_info[decimal_point_adjust_token]
            display_digit_token = f'display_digit_{(sg_gui.token_data).lower()}'
            display_digit_token_value = config_subscan_api_info[display_digit_token]

            # Subscan API設定値の確認
            if address_token_value == "" or decimal_point_adjust_token_value == "" or display_digit_token_value == "":
                sg.popup_ok('ERROR\nSubscan API設定を確認してください。\nアプリを終了します。',font=(font_info,20),button_color='red')
                window.close()
                quit()                
            # 入力値の異常値判定
            if (input_num.isnumeric()) == False or input_num == '' or int(input_num) <= 0:
                sg.popup_ok('ERROR\n件数は1以上の整数で入力してください。アプリを終了します。',font=(font_info,20),button_color='red')
                window.close()
                quit()
            # 件数(文字列)が数を表す文字か判定(1以上が真)
            if input_num.isnumeric():
                if values['-HISTORY-'] == 'Reward&Slash':
                    stkrwd = SubscanStakingRewardDataProcess(input_num,config_subscan_api_info,values['-TOKEN-'],values['-SORT-'])
                    response_code, api_endpoint, response_status_code, header_list ,response_data, df_csv, list_num = stkrwd.get_subscan_stakerewards()
                    #response_code, api_endpoint, response_status_code, header_list ,response_data, df_csv, list_num = subscan.get_subscan_stakerewards(input_num,config_subscan_api_info,values['-TOKEN-'],values['-SORT-'])
                elif values['-HISTORY-'] == 'CryptactCustom':
                    cyrptactcustom = SubscanStakingRewardsDataProcessForCryptact(input_num,config_subscan_api_info,config_cryptact_info,values['-TOKEN-'],values['-SORT-'])
                    response_code, api_endpoint, response_status_code, header_list ,response_data, df_csv, list_num = cyrptactcustom.create_stakerewards_cryptact_cutom_df()
                    #response_code, api_endpoint, response_status_code, header_list ,response_data, df_csv, list_num = subscan.create_stakerewards_cryptact_cutom_df(input_num,config_subscan_api_info,config_cryptact_info,values['-TOKEN-'],values['-SORT-'])

                # 受信データエラー判定
                # codeエラーの場合
                if response_code != 0:
                    text = f'code: {response_code}\n\nError Details: Invalid Account Address.\nPlease Check Account Address:\n'
                    sg.popup_scrolled('error',text, size=(40, 6),font=(font_info,20))
                    window.close()
                    quit()
                
                # HTTP Status Codesエラー
                if response_status_code != 200:
                    text = f'HTTP Status Codes: {response_status_code}\n\nPlease Check Subscan API Documents:\n{config_subscan_api_doc}\n'
                    sg.popup_scrolled('error',text, size=(40, 6),font=(font_info,20)) 
                    window.close()
                    quit()
                
                if list_num == 0:
                    sg.popup_error('値を取得できませんでした。アプリを終了します。',font=(font_info,20))
                    window.close()
                    quit()
                ...
                sg.popup_error('値を取得できませんでした。\nSubscan APIの設定、または入力した情報を確認してください。\nアプリを終了します。',font=(font_info,20))
                window.close()
                quit()
        if event == '-UNSHOW-':
        ...
        if event == '-SETTING-':
            ...
            sg_gui.sort_type = values['-SORT-']
            window.close()
            window = sg_gui.config_window(config_subscan_api_info,values['-TOKEN-'])
        if event == '-CONFIG_UPDATE-':
            ...
            decimal_point_adjust_token_value = f'-SUBSCAN_ADJUST_VALUE_{sg_gui.token_data}-'
            decimal_point_adjust_token = f'adjust_value_{(sg_gui.token_data).lower()}'

            display_digit_token_value = f'-SUBSCAN_DISPLAY_DIGIT_{sg_gui.token_data}-'
            display_digit_token = f'display_digit_{(sg_gui.token_data).lower()}'
            
            edit_subscan[address_token] = values[address_token_value]
            edit_subscan[display_digit_token] = values[display_digit_token_value]
        ...
        if event == '-TABLE-CLICK-':
            ...
            window['-TABLE_DATA-'].Widget.config(readonlybackground=sg.theme_background_color('white'))
