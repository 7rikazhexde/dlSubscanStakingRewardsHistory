...
class SgWindowProcess:
    ...
    def main(self):
        ...
        sg_gui = CreateGui(self.__config_subscan_api_info,self.__config_ui_info,self.__config_cryptact_info)
        ...
        while True:
            ...
            if event == '-SHOW-':
                ...
                decimal_point_adjust_token_value = self.__config_subscan_api_info[decimal_point_adjust_token]
                display_digit_token = f'display_digit_{(sg_gui.token_data).lower()}'
                display_digit_token_value = self.__config_subscan_api_info[display_digit_token]

                # Subscan API設定値の確認
                if address_token_value == "" or decimal_point_adjust_token_value == "" or display_digit_token_value == "":
                    sg.popup_ok('ERROR\nSubscan API設定を確認してください。\nアプリを終了します。',font=(self.__font_info,20),button_color='red')
                    window.close()
                    quit()                
                # 入力値の異常値判定
                if (input_num.isnumeric()) == False or input_num == '' or int(input_num) <= 0:
                    sg.popup_ok('ERROR\n件数は1以上の整数で入力してください。アプリを終了します。',font=(self.__font_info,20),button_color='red')
                    window.close()
                    quit()
                # 件数(文字列)が数を表す文字か判定(1以上が真)
                if input_num.isnumeric():
                    if values['-HISTORY-'] == 'Reward&Slash':
                        stkrwd = SubscanStakingRewardDataProcess(input_num,self.__config_subscan_api_info,values['-TOKEN-'],values['-SORT-'])
                        ...
                        cyrptactcustom = SubscanStakingRewardsDataProcessForCryptact(input_num,self.__config_subscan_api_info,self.__config_cryptact_info,values['-TOKEN-'],values['-SORT-'])
                    ...
                    if response_code != 0:
                        ...
                        sg.popup_scrolled('error',text, size=(40, 6),font=(self.__font_info,20))
                        window.close()
                        quit()
                    ...
                    if list_num == 0:
                        sg.popup_error('値を取得できませんでした。アプリを終了します。',font=(self.__font_info,20))
                        window.close()
                        quit()
            ...
            if event == '-USAGE-':
                self.__font_info = '游ゴシック Medium'
                result = sg.popup_ok_cancel('GithubのREADME.mdへ移動します',font=(self.__font_info,20))
                if result == 'OK':
